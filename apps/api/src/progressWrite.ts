...
export async function writeProgressChange(
  ...
  const session = change.session
    ...
    if (session) {
      ...
      // shape: closed by evidence, in this transaction, or not at all. The
      // learner's own calendar day rides along so the card lands on the day
      // they saw the round happen, not the server's.
      await db.query('select public.complete_matching_planner_items($1, $2, $3)', [
        learnerId, session.id, change.today ?? null,
      ])
    }
