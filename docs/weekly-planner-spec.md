...
**Status:** proposal · **Date:** 2026-09-06 · **Migration:** 0019 (registry in [build-sequence.md](build-sequence.md)) · **Scope:** packages/shared, apps/web, apps/api, one additive migration
...
any open `study` item on **today or an earlier day** for that learner, storing
the session id as evidence. Earlier days, because a learner who does
Tuesday's session on Wednesday has still done it, and the item moves to the
day it actually happened, which the log records as a move followed by a
closure.
...
2. **Comments behind coverage, or free?** Argued both ways in §12.
