...
//
// These live in the shared package rather than the web client because the
// tutor needs them too: a learner adds "US State Capitals" from the catalog
// (`learners.starter_decks`), and the API has to be able to hand that deck
// to an assistant by id, with the same card ids the app writes mastery under.
...
/** The hand-written starter catalog, in the order the app offers it. */
...
/**
 * The starter decks whose ids are in `ids`, in catalog order. Unknown ids are
 * dropped rather than refused: a deck retired from the catalog leaves the
 * learner's row untouched and simply stops appearing.
 */
export function starterDecksFor(ids: readonly string[]): QuizDeck[] {
  if (!ids.length) return []
  return STARTER_DECKS.filter((d) => ids.includes(d.id))
}
